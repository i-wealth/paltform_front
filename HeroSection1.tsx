...
export default function HeroSlider() {
  ...
  return (
    <section style={{fontFamily:"Sans"}} className={`${bgColor} py-0 px-6 md:px-20 relative overflow-hidden transition-colors duration-500`} dir="rtl">
      {/* فلش‌ها */}
      <button
        onClick={prevSlide}
      ...
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 items-center gap-12 text-center md:text-right transition-all duration-500 ease-in-out">
        <div className="md:order-1 order-2 space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-center md:justify-end gap-4 mt-6">
            <button className="bg-blue-500 hover:bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold transition"  onClick={() => handleClick(slide.route)}>
              {slide.button1}
            </button>
            <button className="border border-gray-400 px-6 py-3 rounded-lg font-semibold text-gray-700 hover:bg-gray-100 transition bg-white" onClick={() => handleClick(slide.route)}>
              {slide.button2}
            </button>
          </div>
