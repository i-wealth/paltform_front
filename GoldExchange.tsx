...
export default function CryptoExchange() {
  ...
  const [data, setData] = useState<CryptoItem[]>([
    {
      name: 'طلای آبشده',
      price: 2063000,
      icon: '/images/tala-abshodeh.png',
      description: '  طلای آبشده با حداق  کارمزد ',
    }]);
    ...
    (async () => {
      try {
        ...
        const cryptoList: CryptoItem[] = [
          {
            name: 'طلای آبشده',
            price: Math.round(returnDataItem(prices, "abshodeh").value),
            icon: '/images/tala-abshodeh.png',
            description: '  طلای آبشده با حداق  کارمزد ',
          },
          {
            name: 'سکه تمام',
            price: Math.round(returnDataItem(prices, "sekke").value),
            icon: '/images/seke.PNG',
            description: ' سکه امامی ',
          },
          {
            name: 'صنوق طلای کیان',
            price: Math.round(returnDataItem(prices, "kian").value),
            icon: '/images/gold-sandogh.png',
            description: '  صندوقی برای سرمایه گذاری مطمئن  ',
          },
        ]
        setData(cryptoList);
  ...
  async function handleSell(amnt: number) {
    setLoading(true)
    setError(null)
    if (typeof window !== 'undefined') {
      ...
      if (raw !== null) {
        try {
          ...
          const res = await sellGoldBar(payload)
          setResp(res)
  ...
  const handleBuy = async (amnt: number) => {
    if (typeof window !== 'undefined') {
      const raw = localStorage.getItem('userProfile');
      if (raw !== null) {
        try {
          ...
          const res = await buyGoldBar(payload)
  ...
  async function handleCharge(amnt: number) {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('souercePage', "gold");
        ...
        <div className="space-y-3">
          ...
          {amount && (
            <p className="text-sm text-gray-600 text-right">
              معادل: <span className="font-bold text-green-600">{(amount / active.price).toFixed(6)}</span> {"میلی گرم " + active.name}
            </p>
          )}

          <div className="flex gap-2">
            <button onClick={() => { void handleBuy(Number(amount)) }} className="flex-1 bg-green-600 text-white py-2 rounded-xl hover:bg-green-700">خرید</button>
            <button onClick={() => { void handleSell(Number(amount) / active.price) }} className="flex-1 bg-red-600 text-white py-2 rounded-xl hover:bg-red-700">فروش</button>
          </div>
          <div>
            {error && (
