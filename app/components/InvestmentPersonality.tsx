...
const InvestComparisonList = ({ portfolioName }: { portfolioName: string }) => {
  const data: { label: string; percent: string }[] = {
    'پورتفولیو ارز دیجیتال': [
      { label: 'بیت‌کوین', percent: '+113.7%' },
      { label: 'اتریوم', percent: '+101.5%' },
      { label: 'لایت‌کوین', percent: '+69.0%' },
    ],
    'پورتفولیو طلا': [
      { label: 'سکه', percent: '+110.0%' },
      { label: 'طلا', percent: '+105.5%' },
    ],
    'پورتفولیو ملک': [
      { label: 'ملک مسکونی', percent: '+85.7%' },
      { label: 'ملک تجاری', percent: '+92.5%' },
    ],
  }[portfolioName]

  return (
    <div className="bg-white rounded-lg shadow p-4 mt-6">
      <h3 className="font-bold mb-4 text-center">مقایسه بازدهی {portfolioName}</h3>
...
export default function InvestPage() {
  ...
  // Data for the portfolios
  const portfolios = {
    'پورتفولیو ارز دیجیتال': [45, 30, 25],
    'پورتفولیو طلا': [60, 40, 0],
    'پورتفولیو ملک': [50, 50, 0],
  }

  // Handle portfolio selection
  const handlePortfolioSelect = (portfolioName: string) => {
    setSelectedPortfolio(portfolioName)
  }

  // Handle "Buy Portfolio" action
  const handleBuyPortfolio = () => {
    alert(`پلتفرم برای شما پورتفولیو ${selectedPortfolio} را خریداری خواهد کرد.`)
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      {step === 2 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            {['پورتفولیو ارز دیجیتال', 'پورتفولیو طلا', 'پورتفولیو ملک'].map((portfolio) => (
              <div
                key={portfolio}
                onClick={() => handlePortfolioSelect(portfolio)}
                className="bg-white shadow p-4 rounded-lg cursor-pointer hover:bg-gray-100 transition-all"
              >
                <h3 className="font-semibold text-center">{portfolio}</h3>
                <p className="text-center mt-2">مشاهده ترکیب و مقایسه بازدهی</p>
              </div>
            ))}
          </div>
      ...
      {/* Step 3: Display Portfolio and Comparison List */}
      {step === 3 && selectedPortfolio && (
        <>
          <h3 className="font-bold text-center mt-6">{selectedPortfolio}</h3>
          <DistributionChart data={portfolios[selectedPortfolio]} />
          <InvestComparisonList portfolioName={selectedPortfolio} />
          <button
            onClick={handleBuyPortfolio}
